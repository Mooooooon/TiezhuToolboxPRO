...
第七史诗（Epic Seven）打铁助手：Windows 桌面小工具，通过 **ADB 连接安卓模拟器**（MuMu 12 等）截取游戏画面，用 **PaddleOCR** 识别装备信息（等级、强化等级、名称、品质、主/副属性、套装），按民间算法由副属性计算装备分数，并结合**官方战绩数据（传说分段）**推荐适用角色。
...
├── Program.cs                入口
├── MainForm.cs / .Designer.cs 主界面：顶部工具栏（设备下拉框 + 地址框 + 连接/刷新/目录，
│                             主按钮"截图识别"一步完成截图与识别）；主体左侧装备信息卡片
│                             （名称/等级/分数/强化建议徽章/主副属性/套装，底部左右三件分数阈值可调，
│                             默认 24/24 仅本次运行有效）、右侧适用角色推荐（头像+匹配度）；
│                             截图预览默认隐藏，点"查看截图"从底部展开/收起；
...
├── Modules/Recommend/
│   ├── HeroDatabase.cs       官方角色数据库：加载 Assets/HeroData/heroes.json（传说分段），
│   │                         提供简体套装名→套装代码映射与头像路径
│   ├── HeroRecommender.cs    装备→适用角色推荐：主属性（须为有用属性）与套装（须属主流搭配）为
│   │                         硬门槛，不符直接淘汰；匹配度按副属性的装备分数加权
│   │                         （有用属性得分÷副属性总分，民间算法权重，强化全跳无用属性会显著降分），
│   │                         左三件固定主属性与未识别出的主属性/套装不过滤
│   ├── HeroRecommendation.cs 推荐结果模型
│   └── EnhancementAdvisor.cs 强化建议：分数阶梯（+3 前分数≥阈值，之后每 3 级 +6，+15 时 ≥阈值+30
│                             建议重铸）+ 赌速度阶梯（速度≥3/6/9/12/12 可继续，+15 时 ≥15 建议重铸）；
...
└── Assets/HeroData/          官方战绩数据（采集工具生成，提交进 git）：
    ├── heroes.json           传说分段角色的有用属性 + 主流套装搭配 + 套装名表
...
tools/HeroDataCollector/      官方战绩采集工具（独立项目，不引用主项目）：
                              拉取 STOVE GG 传说分段角色统计 → 推导有用属性/主流套装 →
                              繁转简（assets/TSCharacters.txt，OpenCC）→ 下载头像/套装图标 →
                              写出到 src/TiezhuToolbox/Assets/HeroData/
```
...
cd tools/OcrTest && dotnet run -- --synthetic            # 推荐+强化建议自检（速度套速度鞋→c5154 应 100%；暴击套暴击项链→应淘汰 c5154；
...
- 官方战绩数据：STOVE GG `e7api.onstove.com/gameApi`（POST + query 传参，无需登录），**只采集 `grade_code=legend` 传说分段**避免低分段污染；`Assets/HeroData/` 提交进 git，每个新赛季重跑一次 `tools/HeroDataCollector` 更新。
- 有用属性推导：属性直方图 0+1 号低桶占比 < 15%，或峰值桶 ≥ 4 号桶 ⇒ 玩家普遍在堆此属性；主流套装：使用率 ≥ 10% 才保留。
