...
一款面向《第七史诗》的 Windows 装备强化辅助工具。通过 ADB 连接安卓模拟器并截取游戏画面，使用本地 OCR 自动识别装备信息、计算民间分数、给出强化建议，并结合官方前排分段数据推荐适用角色。
...
- 结合官方前排分段数据，按装备属性与套装推荐适用角色
- 支持编辑每位英雄的有效属性、可用套装和右三件主属性，可屏蔽不参与装备匹配的英雄
- 支持导入、导出英雄装备需求配置，更新官方数据时保留用户自定义内容
- 支持自定义强化阈值、最低角色匹配度、单次自动处理上限、识别快捷键和持续识别间隔
...
可调整装备分数阈值、自动强化处理方式与上限、最低角色匹配度、全局识别快捷键和持续识别间隔，并查看推荐匹配与角色默认配置使用的自动规则。
